...
Enpii Studio adalah monorepo Laravel 12/PHP 8.3+ yang memuat fondasi backend frontend-agnostic untuk aplikasi produk. Repositori ini belum memuat aplikasi produk siap pakai dan tidak menyediakan UI package bersama.
...
UI identity, layout, component library, dan styling milik setiap aplikasi produk. Repositori ini tidak lagi mengembangkan atau menerbitkan `@its-enpii/ui` atau `@its-enpii/skeleton`. Versi paket yang pernah dipublikasikan tetap ada di registry; jangan menghapus atau mengubahnya. Aplikasi lama yang masih memakainya harus memelihara atau memigrasikan UI secara mandiri.

## Mulai berdasarkan peran

| Peran | Mulai dari |
| --- | --- |
| Pengembang aplikasi produk | [Referensi implementasi aktif](docs/implementation-reference.md#5-contoh-integrasi-aplikasi-konsumen) untuk pemasangan Core, tenancy, authorization, dan WhatsApp Client |
| Maintainer Enpii Studio | [Referensi kontrak lengkap](docs/implementation-reference.md) dan matriks kemampuan yang belum ada |
| Operator WhatsApp Gateway | [Setup dan runbook](docs/setup.md), lalu [kontrak API aktif](docs/implementation-reference.md#4-whatsapp-gateway) |
...
| [`services/whatsapp-gateway`](services/whatsapp-gateway) | Layanan Laravel berjaringan | API principal, ACL instance, lifecycle, text send, idempotency, error aman, health/readiness | [WhatsApp Gateway](docs/implementation-reference.md#4-whatsapp-gateway) |
...
Aplikasi produk, resolver tenant konkret, propagasi tenant otomatis ke queue, pengiriman media, dan deployment produksi Gateway **belum ada**. Daftar lengkap beserta pengganti saat ini tersedia pada [matriks kemampuan yang belum ada](docs/implementation-reference.md#8-matriks-kemampuan-yang-belum-ada).
