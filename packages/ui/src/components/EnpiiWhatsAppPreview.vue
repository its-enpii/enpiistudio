...
<template>
  <section
    class="enpii-whatsapp-preview max-w-144 p-4 [background-color:var(--card-bg)] border border-solid [border-width:var(--control-border-width)] [border-color:var(--control-border-color)] rounded-card [color:var(--card-fg)] text-base forced-colors:border-canvas-text max-sm:p-3"
  ...
  >
    <header v-if="showHeader" class="enpii-whatsapp-preview__header flex items-center gap-3 min-h-12 mb-4 py-1 px-2 pb-3 border-b border-solid [border-bottom-color:var(--control-border-color)]">
      <span class="enpii-whatsapp-preview__avatar grid place-items-center w-10 h-10 shrink-0 rounded-full [background-color:var(--assistant-user-msg-bg)] [color:var(--assistant-user-msg-fg)] text-sm font-medium max-xs:w-9 max-xs:h-9" aria-hidden="true">{{ initials }}</span>
      ...
      <span v-if="readOnly" class="enpii-whatsapp-preview__readonly ml-auto [color:var(--tone-neutral-fg)] text-xs font-normal">{{ t('whatsappPreview.readOnly') }}</span>
    </header>
    ...
    <ol role="log" class="enpii-whatsapp-preview__log flex flex-col gap-1 m-0 p-0 list-none">
      <template v-for="(message, index) in messages" :key="message.id">
        <li
          ...
          class="enpii-whatsapp-preview__divider self-center my-2 [color:var(--tone-neutral-fg)] text-xs font-normal"
        >
        ...
        <li class="enpii-whatsapp-preview__item flex" :class="[message.direction === 'out' ? 'justify-end' : 'justify-start', `enpii-whatsapp-preview__item--${message.direction}`]">
          <div class="enpii-whatsapp-preview__bubble relative flex flex-col max-w-4/5 max-w-96 p-3 [background-color:var(--assistant-bot-msg-bg)] border border-solid [border-color:var(--assistant-card-border)] rounded-overlay [color:var(--assistant-bot-msg-fg)] max-sm:max-w-[88%] max-xs:py-2.5 max-xs:px-3" :class="[`enpii-whatsapp-preview__bubble--${message.status === 'failed' ? 'failed' : message.direction === 'out' ? 'out' : 'in'}`, message.status === 'failed' ? '[background-color:var(--tone-danger-soft-bg)] [border-color:var(--tone-danger-border)] [color:var(--tone-danger-soft-fg)]' : message.direction === 'out' ? '[background-color:var(--assistant-user-msg-bg)] border-transparent [color:var(--assistant-user-msg-fg)]' : '']">
            <img v-if="message.mediaUrl" class="enpii-whatsapp-preview__media block w-full max-h-72 mb-2 object-cover rounded-overlay" :src="message.mediaUrl" :alt="message.body">
            ...
            <span class="enpii-whatsapp-preview__meta inline-flex items-center gap-1 self-end mt-1 text-[0.6875rem] font-normal tabular-nums text-current opacity-68">
              ...
              <span v-if="message.direction === 'out' && message.status" class="enpii-whatsapp-preview__status inline-flex" :class="[`enpii-whatsapp-preview__status--${message.status}`, ['read', 'delivered'].includes(message.status) ? '[color:var(--tone-success-fg)]' : message.status === 'failed' ? '[color:var(--tone-danger-fg)]' : '']">
