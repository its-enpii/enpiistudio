    ...
    <ol role="log" class="enpii-whatsapp-preview__log flex flex-col gap-1 m-0 p-0 list-none">
      <template v-for="(message, index) in messages" :key="message.id">
        ...
        <li class="enpii-whatsapp-preview__item flex" :class="[message.direction === 'out' ? 'justify-end' : 'justify-start', `enpii-whatsapp-preview__item--${message.direction}`]">
          <div class="enpii-whatsapp-preview__bubble relative flex flex-col max-w-4/5 max-w-96 p-3 [background-color:var(--assistant-bot-msg-bg)] border border-solid [border-color:var(--assistant-card-border)] rounded-2xl [color:var(--assistant-bot-msg-fg)] max-sm:max-w-[88%] max-xs:py-2.5 max-xs:px-3" :class="[`enpii-whatsapp-preview__bubble--${message.status === 'failed' ? 'failed' : message.direction === 'out' ? 'out' : 'in'}`, message.status === 'failed' ? '[background-color:var(--tone-danger-soft-bg)] [border-color:var(--tone-danger-border)] [color:var(--tone-danger-soft-fg)]' : message.direction === 'out' ? '[background-color:var(--assistant-user-msg-bg)] border-transparent [color:var(--assistant-user-msg-fg)]' : '']">
