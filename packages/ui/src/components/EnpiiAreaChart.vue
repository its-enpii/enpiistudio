...
const ariaLabel = computed(() => {
  ...
  return `Area chart: ${names || 'no data'}`
})
</script>
  ...
  <div class="enpii-area-chart relative w-full">
    <svg :viewBox="`0 0 ${W} ${H}`" class="enpii-area-chart__svg block w-full h-auto select-none" role="img" :aria-label="ariaLabel">
      <line
        ...
        :y1="yAt(tick)"
        :y2="yAt(tick)"
        class="enpii-area-chart__grid [stroke:var(--chart-grid)] stroke-1 opacity-50"
      />
      <text
        v-for="(tick, i) in gridTicks"
        :key="`y-${i}`"
        ...
        class="enpii-area-chart__axis-text [fill:var(--chart-axis)] text-[11px] font-medium"
      ...
      <g v-for="item in series" :key="item.key">
        ...
        <circle
          ...
          :cy="point.y"
          r="4"
          class="enpii-area-chart__point cursor-pointer [stroke:var(--chart-tooltip-bg)] stroke-2 motion-reduce:transition-none focus-visible:[outline-style:var(--tw-outline-style)] focus-visible:[outline-width:var(--focus-width-overlay)] focus-visible:[outline-offset:var(--focus-offset)] focus-visible:outline-focus"
