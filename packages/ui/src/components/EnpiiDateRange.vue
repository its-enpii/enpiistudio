            ...
            <Transition name="dropdown">
                ...
                >
                    ...
                    <div class="enpii-date-range__inputs grid grid-cols-[repeat(2,minmax(0,1fr))] gap-2">
                        <label class="grid gap-1 text-on-surface-variant text-sm"><span>Mulai</span><input class="min-h-control-sm appearance-none px-3 border border-solid border-outline-variant rounded-lg bg-surface-container-lowest text-on-surface font-sans text-control/1.25 focus-visible:outline-none focus-visible:border-primary-container focus-visible:[box-shadow:var(--shadow-focus)]" type="date" :min="min" :max="max" :value="draft.start" @change="commitInput('start', $event)"></label>
                        <label class="grid gap-1 text-on-surface-variant text-sm"><span>Selesai</span><input class="min-h-control-sm appearance-none px-3 border border-solid border-outline-variant rounded-lg bg-surface-container-lowest text-on-surface font-sans text-control/1.25 focus-visible:outline-none focus-visible:border-primary-container focus-visible:[box-shadow:var(--shadow-focus)]" type="date" :min="min" :max="max" :value="draft.end" @change="commitInput('end', $event)"></label>
                    </div>
