    ...
    <nav class="enpii-pagination flex items-center gap-1" aria-label="Pagination">
        ...
        <template v-for="page in pages" :key="typeof page === 'number' ? page : page">
            ...
            <button
                v-else
                type="button"
                class="enpii-pagination__control inline-flex items-center justify-center min-w-control-height-sm h-control-height-sm border border-solid [border-width:var(--control-border-width)] border-transparent rounded-[9999px] bg-transparent font-inherit text-[1rem] cursor-pointer [transition-property:all] duration-fast ease-emphasized focus-visible:outline-3 focus-visible:outline-solid focus-visible:outline-focus focus-visible:outline-offset-2 disabled:opacity-40 disabled:cursor-not-allowed"
