        ...
        <div class="enpii-otp-input__fields flex flex-wrap items-center gap-2">
            <template v-for="(digit, index) in code" :key="`${inputId}-${index}`">
                <input
                    ...
                    class="enpii-otp-input__field w-11 h-11 p-0 border border-solid [border-width:var(--control-border-width)] [border-color:var(--control-border-color)] rounded-control [background-color:var(--field-bg)] [color:var(--field-fg)] [font-family:inherit] text-xl font-medium text-center [transition-property:border-color,box-shadow] duration-fast ease-emphasized hover:enabled:[border-color:var(--field-border)] focus-visible:outline-none focus-visible:[border-color:var(--field-border)] focus-visible:[box-shadow:var(--shadow-focus)] disabled:opacity-60 disabled:cursor-not-allowed"
                    :value="digit"
                ...
                >
                <span v-if="separator && index < length - 1" class="enpii-otp-input__separator [color:var(--field-placeholder-fg)] font-medium" aria-hidden="true">
                    {{ separator }}
                </span>
            </template>
        </div>
