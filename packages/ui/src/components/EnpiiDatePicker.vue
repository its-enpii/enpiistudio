    ...
    <div ref="root" class="enpii-date-picker w-full [&>*+*]:mt-field-gap">
        ...
        <div class="enpii-date-picker__control-wrap relative">
            <button
                ...
                ref="trigger"
                type="button"
                class="enpii-date-picker__control flex w-full h-auto min-h-control items-center px-4 border border-solid [border-width:var(--control-border-width)] [border-color:var(--control-border-color)] rounded-control [background-color:var(--field-bg)] [color:var(--field-fg)] font-sans text-control text-left [transition-property:all] duration-fast ease-emphasized hover:enabled:[border-color:var(--field-border)] active:enabled:[transform:var(--press-transform)] focus:outline-none focus-visible:outline-none focus:[border-color:var(--field-border)] focus-visible:[border-color:var(--field-border)] focus:[box-shadow:var(--shadow-focus)] focus-visible:[box-shadow:var(--shadow-focus)] disabled:opacity-60 disabled:cursor-not-allowed"
            ...
            >
                <AppIcon v-if="icon" :name="icon" class="enpii-date-picker__icon absolute left-4 w-5 h-5 [color:var(--field-placeholder-fg)] pointer-events-none text-xl leading-none" />
                ...
                <AppIcon :name="triggerIcon" class="enpii-date-picker__chevron absolute top-1/2 right-4 w-5 h-5 -translate-y-1/2 [color:var(--field-placeholder-fg)] text-xl leading-none transition-[transform,color] duration-base ease-emphasized" :class="{ 'enpii-date-picker__chevron--open -translate-y-1/2 scale-110 [color:var(--picker-cell-selected-bg)]': open }" />
            </button>
            ...
            <Teleport to="body">
                <Transition name="dropdown">
                    <div
                        ...
                        class="enpii-date-picker__popup min-w-[18rem] p-3 border border-solid [border-color:var(--overlay-border-color)] rounded-control [background-color:var(--overlay-surface-bg)] shadow-overlay select-none origin-top"
                        :class="[
                    ...
                    >
                        <div class="enpii-date-picker__header flex items-center justify-between gap-1 mb-3">
                            <div class="enpii-date-picker__nav-group flex items-center gap-1">
                                <button type="button" class="enpii-date-picker__nav-button grid place-items-center w-8 h-8 p-1 border-0 rounded-lg bg-transparent [color:var(--overlay-surface-fg)] cursor-pointer [transition-property:all] duration-fast ease-emphasized hover:[background-color:var(--picker-cell-hover-bg)] hover:[color:var(--picker-cell-selected-bg)] active:[transform:var(--press-transform)] focus-visible:outline-none focus-visible:[box-shadow:var(--shadow-focus)]" :aria-label="t('datePicker.previousYear')" @click="moveView(0, -1)"><AppIcon name="keyboard_double_arrow_left" class="enpii-date-picker__nav-icon w-5 h-5 text-xl leading-none" /></button>
                                <button type="button" class="enpii-date-picker__nav-button grid place-items-center w-8 h-8 p-1 border-0 rounded-lg bg-transparent [color:var(--overlay-surface-fg)] cursor-pointer [transition-property:all] duration-fast ease-emphasized hover:[background-color:var(--picker-cell-hover-bg)] hover:[color:var(--picker-cell-selected-bg)] active:[transform:var(--press-transform)] focus-visible:outline-none focus-visible:[box-shadow:var(--shadow-focus)]" :aria-label="t('datePicker.previousMonth')" @click="moveView(-1)"><AppIcon name="chevron_left" class="enpii-date-picker__nav-icon w-5 h-5 text-xl leading-none" /></button>
                            </div>
                            <p class="enpii-date-picker__header-title m-0 [color:var(--overlay-surface-fg)] text-sm font-semibold capitalize" aria-live="polite">{{ mode === 'year' ? yearRangeLabel : monthLabel }}</p>
                            <div class="enpii-date-picker__nav-group flex items-center gap-1">
                                <button type="button" class="enpii-date-picker__nav-button grid place-items-center w-8 h-8 p-1 border-0 rounded-lg bg-transparent [color:var(--overlay-surface-fg)] cursor-pointer [transition-property:all] duration-fast ease-emphasized hover:[background-color:var(--picker-cell-hover-bg)] hover:[color:var(--picker-cell-selected-bg)] active:[transform:var(--press-transform)] focus-visible:outline-none focus-visible:[box-shadow:var(--shadow-focus)]" :aria-label="t('datePicker.nextMonth')" @click="moveView(1)"><AppIcon name="chevron_right" class="enpii-date-picker__nav-icon w-5 h-5 text-xl leading-none" /></button>
                                <button type="button" class="enpii-date-picker__nav-button grid place-items-center w-8 h-8 p-1 border-0 rounded-lg bg-transparent [color:var(--overlay-surface-fg)] cursor-pointer [transition-property:all] duration-fast ease-emphasized hover:[background-color:var(--picker-cell-hover-bg)] hover:[color:var(--picker-cell-selected-bg)] active:[transform:var(--press-transform)] focus-visible:outline-none focus-visible:[box-shadow:var(--shadow-focus)]" :aria-label="t('datePicker.nextYear')" @click="moveView(0, 1)"><AppIcon name="keyboard_double_arrow_right" class="enpii-date-picker__nav-icon w-5 h-5 text-xl leading-none" /></button>
                            </div>
                        </div>

                        <Transition name="calendar-view" mode="out-in">
                            <div v-if="mode === 'date'" key="view-date" role="grid" :aria-label="monthLabel">
                                <div role="row" class="enpii-date-picker__weekdays grid grid-cols-7 mb-1"><span v-for="weekday in weekdays" :key="weekday" role="columnheader" class="enpii-date-picker__weekday py-1 [color:var(--label-fg)] text-xs font-semibold text-center">{{ weekday }}</span></div>
                                <div class="enpii-date-picker__days grid grid-cols-7 gap-y-1">
                                    <button v-for="day in days" :key="day.iso" type="button" role="gridcell" :data-date="day.iso" :tabindex="day.iso === toIso(focusedDate) ? 0 : -1" :aria-selected="day.iso === model" :aria-current="day.iso === today ? 'date' : undefined" :disabled="day.disabled" class="enpii-date-picker__day grid place-items-center w-9 h-9 mx-auto border-0 rounded-[9999px] bg-transparent [color:var(--overlay-surface-fg)] text-sm cursor-pointer [transition-property:all] duration-fast ease-emphasized hover:enabled:[background-color:var(--picker-cell-hover-bg)] focus-visible:outline-none focus-visible:[box-shadow:var(--shadow-focus)] disabled:cursor-not-allowed disabled:opacity-40" :class="{
                                        'enpii-date-picker__day--selected ![background-color:var(--picker-cell-selected-bg)] ![color:var(--picker-cell-selected-fg)] font-semibold': day.iso === model,
                                        'enpii-date-picker__day--today [background-color:var(--picker-range-bg)] [color:var(--picker-cell-selected-bg)] font-semibold': day.iso !== model && day.iso === today,
                                        'enpii-date-picker__day--outside opacity-40': day.currentMonth === false,
                            ...
                            <div v-else-if="mode === 'month'" key="view-month" role="grid" :aria-label="monthLabel">
                                <div class="enpii-date-picker__cells grid grid-cols-3 gap-2">
                                    <button v-for="item in months" :key="item.value" type="button" role="gridcell" :aria-selected="item.isSelected" class="enpii-date-picker__cell py-3 px-3 border-0 rounded-lg bg-transparent [color:var(--overlay-surface-fg)] text-sm font-semibold cursor-pointer [transition-property:all] duration-fast ease-emphasized hover:[background-color:var(--picker-cell-hover-bg)] focus-visible:outline-none focus-visible:[box-shadow:var(--shadow-focus)] active:scale-95" :class="{ 'enpii-date-picker__cell--selected [background-color:var(--picker-cell-selected-bg)] [color:var(--picker-cell-selected-fg)]': item.isSelected, 'enpii-date-picker__cell--current [background-color:var(--picker-range-bg)] [color:var(--picker-cell-selected-bg)]': !item.isSelected && item.isCurrent }" @click="chooseMonth(item)">{{ item.label }}</button>
                                </div>
                            </div>

                            <div v-else key="view-year" role="grid" :aria-label="yearLabel">
                                <div class="enpii-date-picker__cells grid grid-cols-3 gap-2">
                                    <button v-for="year in yearColumn" :key="year.value" type="button" role="gridcell" :aria-selected="year.isSelected" class="enpii-date-picker__cell py-3 px-3 border-0 rounded-lg bg-transparent [color:var(--overlay-surface-fg)] text-sm font-semibold cursor-pointer [transition-property:all] duration-fast ease-emphasized hover:[background-color:var(--picker-cell-hover-bg)] focus-visible:outline-none focus-visible:[box-shadow:var(--shadow-focus)] active:scale-95" :class="{ 'enpii-date-picker__cell--selected [background-color:var(--picker-cell-selected-bg)] [color:var(--picker-cell-selected-fg)]': year.isSelected, 'enpii-date-picker__cell--current [background-color:var(--picker-range-bg)] [color:var(--picker-cell-selected-bg)]': !year.isSelected && year.isCurrent }" @click="chooseYear(year)">{{ year.label }}</button>
                                </div>
                            </div>
                        </Transition>
                        ...
                        <div class="enpii-date-picker__footer flex items-center justify-between mt-3 pt-3 border-t border-solid [border-color:var(--overlay-border-color)] text-sm font-semibold">
                            <button v-if="clearable && model" type="button" class="enpii-date-picker__footer-button enpii-date-picker__footer-button--clear py-2 px-3 border-0 rounded-lg bg-transparent [color:var(--tone-danger-fg)] cursor-pointer [transition-property:all] duration-fast ease-emphasized hover:[background-color:var(--tone-danger-soft-bg)] active:[transform:var(--press-transform)]" @click="clear">{{ t('datePicker.clear') }}</button>
                            <span v-else class="enpii-date-picker__footer-spacer w-16" />
                            <button type="button" class="enpii-date-picker__footer-button py-2 px-3 border-0 rounded-lg bg-transparent [color:var(--picker-cell-selected-bg)] cursor-pointer [transition-property:all] duration-fast ease-emphasized hover:[background-color:var(--picker-cell-hover-bg)] active:[transform:var(--press-transform)]" @click="selectToday">{{ todayActionLabel }}</button>
                        </div>
                    </div>
                </Transition>
            </Teleport>
        </div>
        <p v-if="error" :id="`${inputId}-error`" class="enpii-date-picker__help enpii-date-picker__help--error ml-1 [color:var(--field-error-fg)] text-[0.8125rem]">{{ error }}</p>
        <p v-else-if="hint" :id="`${inputId}-hint`" class="enpii-date-picker__help ml-1 [color:var(--label-fg)] text-[0.8125rem]">{{ hint }}</p>
    </div>
</template>
