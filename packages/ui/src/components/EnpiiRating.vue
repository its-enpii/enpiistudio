    ...
    >
        ...
        >
            <button
                ...
                :key="value"
                type="button"
                class="enpii-rating__star grid place-items-center p-0 border-0 bg-transparent [color:var(--tone-neutral-border)] rounded-sm cursor-pointer [transition-property:color,transform] duration-fast ease-standard motion-reduce:transition-none hover:enabled:[color:var(--tone-warning-fg)] hover:enabled:scale-105 focus-visible:[outline-style:var(--tw-outline-style)] focus-visible:[outline-width:var(--focus-width-overlay)] focus-visible:[outline-offset:var(--focus-offset)] focus-visible:outline-focus disabled:cursor-default"
                :class="{ 'enpii-rating__star--active ![color:var(--tone-warning-fg)]': value <= previewValue }"
                :disabled="readonly"
                :tabindex="-1"
