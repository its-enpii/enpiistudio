...
<template>
    <div class="enpii-rich-editor overflow-hidden border border-solid border-outline-variant [border-width:var(--control-border-width)] rounded-control bg-surface-container-lowest focus-within:border-primary-container focus-within:[box-shadow:var(--shadow-focus)]" :class="[shapeClass, { 'enpii-rich-editor--disabled border-danger-border opacity-65': disabled }]">
        <div class="enpii-rich-editor__toolbar flex flex-wrap items-center gap-1 p-1 border-b border-solid border-outline-variant [border-bottom-width:var(--control-border-width)] bg-surface-container-low" role="toolbar" :aria-label="t('richEditor.toolbarLabel')">
            <button v-for="item in commands" :key="item.key" type="button" class="enpii-rich-editor__btn enpii-rich-editor__btn--ghost-sm grid place-items-center w-8 h-8 border border-transparent rounded-lg bg-transparent text-on-surface-variant cursor-pointer [transition-property:background,color,border-color,transform] duration-fast ease-emphasized motion-reduce:transition-none hover:enabled:bg-surface-container-high hover:enabled:text-primary-text active:enabled:[transform:scale(.96)] focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-focus disabled:cursor-not-allowed disabled:opacity-45 [&_svg]:w-[1.125rem] [&_svg]:h-[1.125rem]" :title="item.label" :aria-label="item.label" :disabled="disabled" @mousedown.prevent @click="execute(item.command)">
                <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path :d="item.icon" /></svg>
            </button>
            <span class="enpii-rich-editor__sep w-px h-5 mx-1 bg-outline-variant" />
        ...
        <div class="enpii-rich-editor__surface relative cursor-text" :class="{ 'cursor-not-allowed': disabled }" @click="focusEditor">
            <EditorContent :editor="editor" class="enpii-rich-editor__content-wrapper min-h-36 p-4 text-on-surface text-sm leading-[1.6] outline-none [&_.ProseMirror]:outline-none [&_h1]:m-0 [&_h1]:mb-3 [&_h1]:text-xl [&_h1]:font-semibold [&_h1]:leading-[1.5] [&_h2]:m-0 [&_h2]:mb-3 [&_h2]:text-lg [&_h2]:font-semibold [&_h2]:leading-[1.5] [&_h3]:m-0 [&_h3]:mb-3 [&_h3]:text-base [&_h3]:font-semibold [&_h3]:leading-[1.5] [&_h4]:m-0 [&_h4]:mb-3 [&_h4]:text-[0.9375rem] [&_h4]:font-semibold [&_h4]:leading-[1.5] [&_h5]:m-0 [&_h5]:mb-3 [&_h5]:text-[0.9375rem] [&_h5]:font-semibold [&_h5]:leading-[1.5] [&_h6]:m-0 [&_h6]:mb-3 [&_h6]:text-[0.9375rem] [&_h6]:font-semibold [&_h6]:leading-[1.5] [&_p]:m-0 [&_p]:mb-3 [&_p]:font-normal [&_p]:text-control [&_p]:leading-[1.5] [&_ul]:m-2 [&_ul]:mb-3 [&_ul]:pl-6 [&_ol]:m-2 [&_ol]:mb-3 [&_ol]:pl-6 [&_blockquote]:pl-4 [&_blockquote]:border-l-[3px] [&_blockquote]:border-outline-variant [&_blockquote]:font-normal [&_blockquote]:not-italic [&_hr]:h-px [&_hr]:my-4 [&_hr]:border-0 [&_hr]:bg-outline-variant [&_pre]:overflow-x-auto [&_pre]:p-3 [&_pre]:rounded-lg [&_pre]:bg-surface-container-low [&_code]:font-mono [&_code]:font-normal [&_code]:whitespace-pre-wrap [&_a]:text-primary-text [&_a]:underline [&_.is-editor-empty:first-child]:before:content-[attr(data-placeholder)] [&_.is-editor-empty:first-child]:before:pointer-events-none [&_.is-editor-empty:first-child]:before:absolute [&_.is-editor-empty:first-child]:before:top-4 [&_.is-editor-empty:first-child]:before:left-4 [&_.is-editor-empty:first-child]:before:text-on-surface-variant [&_.is-editor-empty:first-child]:before:opacity-75" />
        </div>
        <slot name="toolbar" />
    </div>
</template>
