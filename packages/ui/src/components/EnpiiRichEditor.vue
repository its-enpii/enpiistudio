...
<template>
    <div class="enpii-rich-editor overflow-hidden border border-solid border-outline-variant rounded-control bg-surface-container-lowest focus-within:border-primary-container focus-within:[box-shadow:var(--enpii-focus-ring)]" :class="[shapeClass, { 'enpii-rich-editor--disabled border-danger-border opacity-65': disabled }]">
        <div class="enpii-rich-editor__toolbar flex flex-wrap items-center gap-1 p-1 border-b border-solid border-outline-variant bg-surface-container-low" role="toolbar" :aria-label="t('richEditor.toolbarLabel')">
            <button v-for="item in commands" :key="item.key" type="button" class="enpii-rich-editor__btn enpii-rich-editor__btn--ghost-sm grid place-items-center w-8 h-8 border border-transparent rounded-lg bg-transparent text-on-surface-variant cursor-pointer [transition-property:background,color,border-color,transform] duration-fast ease-emphasized motion-reduce:transition-none hover:enabled:bg-surface-container-high hover:enabled:text-primary-text active:enabled:scale-96 focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-focus disabled:cursor-not-allowed disabled:opacity-45 [&_svg]:w-[1.125rem] [&_svg]:h-[1.125rem]" :title="item.label" :aria-label="item.label" :disabled="disabled" @mousedown.prevent @click="execute(item.command)">
