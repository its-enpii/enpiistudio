    ...
    <Teleport to="body">
        <Transition name="theme-menu">
            <div
                ...
                class="enpii-theme-menu__panel fixed right-4 top-[4.5rem] z-modal w-64 origin-top-right overflow-hidden rounded-overlay border [border-color:var(--overlay-border-color)] [border-width:var(--overlay-border-width)] [background-color:var(--overlay-surface-bg)] shadow-overlay"
            >
                ...
                <div class="enpii-theme-menu__options p-1">
                    <button
                        ...
                        class="enpii-theme-menu__option flex w-full cursor-pointer items-center gap-3 rounded-control border-0 bg-none px-3 py-2 text-left text-sm transition-all duration-fast ease-emphasized hover:[background-color:var(--control-bg-hover)] focus-visible:outline-none focus-visible:[box-shadow:var(--shadow-focus)]"
                        :class="{ '[background-color:var(--control-bg-hover)] font-semibold': theme === themeOption.id }"
                        @click="choose(themeOption.id)"
                    >
                        <span class="enpii-theme-menu__swatch inline-grid h-6 w-9 shrink-0 grid-flow-col gap-1 overflow-hidden rounded-control border [border-color:var(--control-border-color)] [border-width:var(--control-border-width)] p-1" :data-for="themeOption.id" aria-hidden="true"><i class="[background-color:var(--tone-primary-bg)]" /><i class="[background-color:var(--card-bg)]" /><i class="[background-color:var(--card-fg)]" /></span>
