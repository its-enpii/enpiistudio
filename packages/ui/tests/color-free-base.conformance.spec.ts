...
const colorFreeBatches: Record<'batch1' | 'batch2', { components: string[]; hooks: string[] }> = {
  batch1: {
...
function isAllowedColorDeclaration(property: string, value: string) {
  ...
  return [
    ...
    'transparent',
    'currentcolor',
  /^var\(--(?:control|field|label|tone|overlay|card|nav|skeleton|progress|table|kanban)-[a-z0-9-]+\)$/,
  ...
  it.each(colorFreeBatches.batch1.components)('batch 1: %s emits no color decisions outside batch hooks', async (filename) => {
    ...
    for (const className of [...classes].sort()) {
      ...
      for (const declaration of css.matchAll(/(?:^|[;{])\s*([a-z-]+)\s*:\s*([^;}]+)/gi)) {
        ...
        if (/var\(--(?:control|field|label|tone|overlay|card|nav|skeleton|progress|table|kanban)-[a-z0-9-]+\)/.test(value.trim())) continue
