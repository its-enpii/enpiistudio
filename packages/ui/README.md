...
Tokens berada di custom properties `--enpii-*`. Tailwind consumer dapat memetakan token pada theme sendiri tanpa package ini memaksa versi atau konfigurasi Tailwind. Palet indigo, semantic emerald/amber/red/slate, radius, spacing, serta font stack mengacu secara read-only pada Encore lokal. Kekurangan aksesibilitas referensi—button tanpa type, icon-only tanpa label, animated status, serta focus non-`visible`—tidak direplikasi.
